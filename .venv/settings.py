...
class SettingsWindow(cck.CTkToplevel):
    def __init__(self, parent, theme_manager):
        ...
        for label, val in [("Тёмная (стандарт)", "dark"), ("AMOLED (глубокий чёрный)", "amoled"), ("Тёмно-синяя", "dark_blue")]:
            cck.CTkRadioButton(t_box, text=label, variable=self.t_var, value=val, command=self._on_theme).pack(anchor="w", padx=20, pady=4)
        ...
        cck.CTkLabel(f_box, text="🔤 Размер шрифта истории транзакций", font=cck.CTkFont(weight="bold")).pack(pady=(8, 2))
        ...
        self.cur_opt = cck.CTkOptionMenu(c_box, values=["₽ (RUB)", "$ (USD)", "€ (EUR)", "¥ (CNY)"], command=self._on_curr_change)
    ...
    def _on_theme(self):
        ...
        # Принудительно восстанавливаем шрифт после сброса стилей темой
